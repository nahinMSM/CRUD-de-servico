...
export const Governanca = () => {
  ...
  window.addEventListener("DOMContentLoaded", async () => {
    ...
    onGetTasks((querySnapshot) => {
        ...
        if (task.status == "Aberto") {
          tasksList.innerHTML +=
            ...
            <div class="status">
              ...
              <button class="btn-excluir bg" id="edit" data-id="${doc.id}">Excluir</button>
            </div>
          </div>`
        }
      ...
      const btnsEdit = tasksList.querySelectorAll(".btn-excluir")
      btnsEdit.forEach((btn) => {
        btn.addEventListener("click", async (e) => {
          const doc = await getTask(e.target.dataset.id)
          const task = doc.data()
          openModal()
          taskForm.querySelector(`option[value="Aberto"]`).style.display = "none"
          ...
          taskForm["status-confirm"].value = "Excluir"
