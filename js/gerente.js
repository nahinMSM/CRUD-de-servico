...
export const Gerente = () => {
  ...
  window.addEventListener("DOMContentLoaded", async () => {
    ...
    onGetTasks((querySnapshot) => {
          ...
          tasksList.innerHTML +=
            `<div class="res">
                ...
                <div class="status">
                  ...
                  <button class="btn-editStatusAberto bg" id="edit" data-id="${doc.id}"> Editar</button>
                  <button class="deletStatusAberto bg" data-id="${doc.id}">Deletar</button>
        ...
        if (task.status == "Interditado") {
          tasksListInterditado.innerHTML +=
            ...
            <div class="status">
              <div class="status-box">
                <h3>Status:</h3>
                <div class="bgStatus interditado" style="background-color:yellow;"><b>&#161;</b> ${task.status}</div>
              </div>
              <div class="por">
                <strong>Por:</strong> <p>${task.por}</p>
                <button class="btn-delete bg" data-id="${doc.id}">Deletar</button>
              </div>
            </div>
          </div>`
        }
        ...
        if (task.status == "Concluído") {
          ...
          <div class="status">
            <div class="status-box">
              ...
              <button class="btn-delete bg bgStatus" data-id="${doc.id}" style="background-color:red;">&#10004; ${task.status}</button> 
      ...
      const btnEditAberto = document.querySelectorAll(".btn-editStatusAberto")
      btnEditAberto.forEach((btn) => {
        btn.addEventListener("click", async (e) => {
          const doc = await getTask(e.target.dataset.id)
          const task = doc.data()
          openModal()
          taskForm.querySelector(`option[value="Interditado"]`).style.display = "none"
          taskForm.querySelector(`option[value="Concluído"]`).style.display = "none"
      ...
      const btnDelete = document.querySelectorAll(".btn-delete, .deletStatusAberto")
